...
class AuthService{
  ...
  void getUser({
    required BuildContext context
      ...
      try{
        ...
        http.Response validity = await http.get(Uri.parse('$uri/api/is-token-valid'), headers:<String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
  ...
  void createUser({
    ...
    required BuildContext context
      ...
      try{
        User user = User(email: email, username: username, password: password, user_id: 0,token:' ');
        http.Response respnose = await http.post(Uri.parse('$uri/api/signup'), body: user.toJson(), headers:<String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        });
        httpErrorHandle(response: respnose, context: context, onSuccess: ()async{
          Provider.of<UserProvider>(context, listen:false).setUser(jsonDecode(respnose.body)[0]);
          Navigator.pushNamed(context, GroupList.routeName);
    });
      }
      catch(e){
